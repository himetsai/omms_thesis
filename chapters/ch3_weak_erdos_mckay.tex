...
For any $C > 0$ there exists $c = c(C) > 0$ such that the following holds. For any $C$-Ramsey graph $G$ on $n$ vertices and any $m$ satisfying $cn^2 \le m \le 2cn^2$, there is a vertex set $U_0 \subseteq V(G)$ with $|e(U_0) - 4m| = O(n^{3/2})$ and sets $\mathbf{S}$, $\mathbf{T}$, $\mathbf{X}$ of size $\Theta(\sqrt{n})$ such that
\begin{enumerate}
    ...
    \item $\min_{\mathbf{x} \in \mathbf{T}} d_{U_0}(\mathbf{x}) -
        \max_{\mathbf{x} \in \mathbf{S}} d_{U_0}(\mathbf{x}) \geq 8c'\sqrt{n}$ for some constant $c' > 0$,
...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \]
    for large enough $Q$ (note that enlarging $Q$ does not fail Property~1).
  \end{enumerate}
\end{proof}
