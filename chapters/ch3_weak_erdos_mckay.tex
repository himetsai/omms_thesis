...
We also show the parallel result of Lemma \ref{lemma:diverse-subgraph} that every $C$-Ramsey graph contains an $(\eps, \delta)$-rich subgraph of linear size, following a similar line of argument.
...
\begin{proof}
  ...
  \]
  Note that $|S| \geq (cn)^{\delta}$, and so Theorem \ref{thm:ramsey-density} yields a homogeneous set $A$ in $S$ of size 
  \[
    |A| \geq \frac{\gamma K}{6 \log (K/6)} \log |S| \geq \frac{\gamma\delta K}{6 \log (K/6)} \log cn = \frac{\gamma\delta K}{6 \log (K/6)} \log n + o(\log n)
  \]
  where $\gamma > 0$ is an absolute constant given by the theorem. But then $\frac{\gamma\delta K}{6 \log (K/6)} > C$ for sufficiently large $K$, and so $|A| > C \log n$, contradiction.
