...
The approach fundamentally shifts from bounding degree-collision probabilities for pairs of vertices to directly controlling the point probabilities of $e(U)$ itself. This requires a substantially richer set of tools, but the underlying principle remains: when a random variable is approximately normal with large standard deviation, its point probabilities are small.

\subsection{Point probabilities of $e(U)$}

Let $U$ be a uniformly random vertex subset. The random variable $e(G[U])$ has mean $e(G)/2 = \Theta(n^2)$. For variance, the standard variance identity gives
\[
  \Var(e(U)) = \sum_{(e, e') \in E(G)^2} \Cov(\mathbbm{1}_{e \subseteq U}, \mathbbm{1}_{e' \subseteq U}).
\]
The covariance is zero if $e$ and $e'$ are disjoint and $\Theta(1)$ otherwise, so $\Var(e(U)) = O(n^3)$. For a fixed vertex $v$, the number of pairs of edges sharing $v$ is $\binom{\deg(v)}{2}$. Since Ramsey graphs have density bounded away from $0$ and $1$ by Theorem~\ref{thm:ramsey-density}, Jensen's inequality gives
\[
...
By Theorem \ref{thm:alks2003}, there exists $\alpha = \alpha(C) > 0$ such that the theorem holds for all $0 \leq x \leq n^\alpha$. Thus we may assume $x \geq n^\alpha$. Let $m$ be the smallest integer such that $e(\{1, \ldots, m\}) \ge x/4$ and put $G' = G[\{1, \ldots, m\}]$. Then 
\[
...
\]
Therefore, there exists $U \subseteq V(G') \subseteq V(G)$ such that $e(G[U)] = e(G'[U]) = x$, for sufficiently large $n$. \qed

\subsection{Proof overview}
\label{sec:proof-overview}

The standard way to approach local limit theorems is via Fourier transforms. Assume $V(G) = [n]$ and define the \emph{graph polynomial}
\[
...
\]
which is of order $\Theta(1/\sigma)$ for any $x$ within $O(\sigma)$ of the mean. Hence, if $|\varphi_X(\tau) - \varphi_Z(\tau)|$ is small, then $\prob[X = x]$ inherits the same order of magnitude $\Theta(1/\sigma)$ through \eqref{eq:fourier-inversion}. If a local central limit theorem held for $X = e(U)$, the proof would amount to verifying suitable estimates on $|\varphi_X(\tau)|$ on $[-\pi, \pi]$ and we are done.

Unfortunately, an actual local central limit theorem does not hold for $e(U)$ in general. The distribution of $e(U)$ can fail to be smooth at the unit scale despite the global shape being approximately Gaussian, which results in spikes in the characteristic function and prevents the integrand in \eqref{eq:fourier-inversion} from being uniformly small. 
...
\]
for some constant $B = B(C)$. The second stage upgrades it to a pointwise estimate by showing that the probability mass cannot be too unevenly distributed within the interval via a switching argument.
...
\begin{equation}
  ...
  \prob[|X - x| \le B] = \Theta(n^{-3/2}),
\end{equation}
but leaves open the worst case where all of this mass sits on a single integer $x' \ne x$ inside the window $[x - B, x + B]$. We now upgrade the interval estimate to a pointwise bound by showing that the mass cannot be too unevenly distributed across the window via a \textit{switching argument} between different possibilities of the set $U$. In particular, \eqref{eq:interval-estimate} already implies an upper bound on $\prob[X = x]$, so it remains to show a lower bound.
...
\]
It remains to construct such a set $T$. Just as in Chapter~\ref{sec:ch3}, we can use the richness of Ramsey graphs to ensure $|N(y) \triangle N(z)| = \Theta(n)$. In particular, we apply Lemma~\ref{lem:richness-extraction} with parameter $\eps = \eps(C) > 0$ and a sufficiently small $\delta < \eps$ to obtain a linear size vertex subset $S_0$ that induces an $(\eps, \delta)$-rich subgraph. Applying richness twice then shows that, for all but $O(|S_0|^{1 + \delta})$ ordered pairs $(y, z) \in S_0^2$,
\begin{equation}
  \label{eq:switching-richness}
  |(N(y) \setminus N(z)) \cap S_0| \geq \eps^2 |S_0|, \qquad  |(N(z) \setminus N(y)) \cap S_0| \geq \eps^2 |S_0|.
\end{equation}
More specifically, by richness all but at most $|S_0|^\delta$ vertices $z \in S_0$ satisfy $|N(z) \cap S_0| \geq \eps|S_0|$. Since $|N(z) \cap S_0| \geq \eps|S_0| \geq \delta|S_0|$, applying richness a second time with set $N(z) \cap S_0$ gives
\[
  |(N(z) \setminus N(y)) \cap S_0| = |\overline{N(y)} \cap (N(z) \cap S_0)| \geq \eps \cdot |N(z) \cap S_0| \geq \eps^2 |S_0|
\]
for all but a further $|S_0|^\delta$ choices of $y \in S_0$. The bound for $N(y) \setminus N(z)$ follows symmetrically. Combining both bounds in \eqref{eq:switching-richness} implies $|N(y) \triangle N(z)| = \Theta(n)$ for all but $O(n^{1 + \delta})$ ordered pairs $(y, z) \in S_0^2$. Now partitioning the degree sequence into $\sqrt n$ intervals and taking $S$ to be the largest bin of vertices from $S_0$ satisfies the $\deg(z) - \deg(y) = \Theta(\sqrt{n})$ condition. Define $T$ as the set of ordered pairs $(y, z) \in S^2$ with $y \neq z$ that satisfy~\eqref{eq:switching-richness}. The first moment estimate~\eqref{eq:moment-1} now follows. 

The second moment bound~\eqref{eq:moment-2} is more involved. Write $A_{y, z} = \{y \in U,\, z \notin U,\, s(y, z; U) = \ell\}$ and expanding $Y_\ell^2$ gives
\[
  \E\bigl[Y_\ell^2 \cdot \mathbbm{1}_{|X - x| \le B}\bigr]
  = \sum_{(y_1, z_1), (y_2, z_2) \in T}
     \prob\bigl[A_{y_1, z_1} \cap A_{y_2, z_2} \cap \{|X - x| \le B\}\bigr].
\]
We would like bound this from above by $\prob[A_{y_1, z_1}] \cdot \prob[A_{y_2, z_2}] \cdot \prob[|X - x| \le B]$. This is achieved via \eqref{eq:switching-richness} and the \textit{dependent random choice} technique -- see~\cite[Lemmas~13.1, 13.4, 13.9]{Kwan_2023} for the details.

\textit{Remark.} The inequality \eqref{eq:CS-main} is stated with the indicator $\mathbbm{1}_{X = x}$, while \eqref{eq:moment-1} and \eqref{eq:moment-2} are stated for the wider event $\{|X - x| \le B\}$. To bridge the two, Cauchy--Schwarz is applied in \cite{Kwan_2023} a second time to the higher-order moment $\E[Y_{-B} Y_{-B + 1} \cdots Y_B \cdot \mathbbm{1}_{|X - x| \le B}]$, which averages over the choice of $\ell$. We do not pursue this iteration here, as the conceptual core remains the comparison \eqref{eq:CS-main}.
