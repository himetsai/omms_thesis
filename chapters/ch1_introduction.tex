...
This dissertation will start by studying the result of Bukh and Sudakov \cite{BUKH2007612} on large induced subgraphs of Ramsey graphs with many distinct degrees. This work not only tighten the connection between Ramsey graphs and dense random graphs, but also introduces a key feature of Ramsey graphs that unlocks the subsequent $|\Phi(G)| \Omega(n^2)$ result by Kwan and Sudakov, which we will present in the following chapter. At the end, we go through the broad strokes of the solution to the Erdős-Mckay conjecture by Kwan, Sah, Sauermann, and Sawhney \cite{Kwan_2023}, touching on the core ideas of the proof.
